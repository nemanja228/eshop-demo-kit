...
- **Model drift at demo-base (verified 2026-08-03 by the 1.3 probe):** the committed 2021-era `CatalogContextModelSnapshot` differs from what EF 8 computes from today's model (nullable-reference-types era), so **any new migration scaffolds unrelated `ALTER COLUMN ... NOT NULL` changes** (Orders/OrderItems/Catalog columns) plus an "operation may result in the loss of data" warning. Every run that adds a migration sees this noise; `dotnet ef migrations remove` also regenerates the snapshot textually (restore it from git). Rubric C6 treats this as baseline.
